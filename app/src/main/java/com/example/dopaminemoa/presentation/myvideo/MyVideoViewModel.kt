...
class MyVideoViewModel(private val apiService: RemoteDataSource) : ViewModel() {
    ...
    fun getBookmarkedItems(context: Context) {
        ...
        _likedItems.value = Utils.getPrefBookmarkItems(context)
    }
    // 특정 아이템을 삭제하는 함수
    fun deleteItem(context: Context, item: VideoItemModel, position: Int) {
        // Utils 클래스를 이용해 아이템 삭제
        Utils.deletePrefItem(context, item.id)
        Log.d("VideoItemModel", "dopaminemoa deleteItem position=${position}, id = ${item.id}")
